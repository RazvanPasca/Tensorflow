...
def generator(z, reuse=tf.AUTO_REUSE):
    with tf.variable_scope('gen', reuse=reuse):
        with tf.variable_scope("linear"):
            linear = clayers.fully_connected(z, 128 * 4 * 4)

        with tf.variable_scope("conv1_transp"):
            # Reshape as 4x4 images
            conv1 = tf.reshape(linear, (-1, 4, 4, 128))
...
with tf.Session() as sess:
    ...
    for step in range(epochs):
        ...
        for b in range(num_batches//25):
            # get images, reshape and rescale to pass to D
            batch = mnist.train.next_batch(batch_size)
            batch_images = tf.image.resize_images(batch[0].reshape(-1, 28, 28, 1),(64,64))
            batch_images = batch_images * 2 - 1

            # generate noise for G
            batch_z = np.random.uniform(-1, 1, size=(batch_size, 100))
            result = sess.run(batch_images)
            # run optimizers
            _ = sess.run(D_trainer, feed_dict={real_images: result, z: batch_z})
    ...
    save_path = saver.save(sess, "./models/200_epoch_model_DCGAN2.ckpt")

print(samples[0].shape)
plt.imshow(samples[0].reshape(28, 28), cmap='Greys')
plt.show()

thefile = open('test_DCGAN2.txt', 'w')
for item in samples:
    thefile.write("%s\n" % item)
...
new_samples = []
with tf.Session() as sess:
    saver.restore(sess, './models/200_epoch_model_DCGAN2.ckpt')
...
for sample in new_samples:
    plt.imshow(sample[0].reshape(64, 64), cmap='Greys')
plt.show()
